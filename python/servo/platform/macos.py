...
class MacOS(Base):
    ...
    def _platform_bootstrap(self, force: bool, yes: bool) -> bool:
        installed_something = (self._platform_bootstrap_brew or self._platform_bootstrap_macports)

        if installed_something:
            print("Could not install dependencies. Make sure you have Homebrew or MacPorts installed.")
            return False
    ...
    def _platform_bootstrap_brew(self) -> bool:
        ...
        print(output)
        return "Installing" in output

    def _platform_bootstrap_macports(self) -> bool:
        print("Trying MacPorts...")
        macportsScript = os.path.join(os.path.dirname(__file__), "macports.sh")
        try:
            result = subprocess.run([macportsScript], check=True)
        except subprocess.CalledProcessError:
            return False
